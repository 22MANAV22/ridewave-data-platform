#   BLOCK 8 — DATABRICKS SILVER LAYER
#   Time    : ~2 hours
#   Platform: Databricks Workspace → Serverless
#   WHY     : Bronze has raw messy data — nulls, wrong types, mixed case.
#             Silver applies business rules to clean it.
#             MERGE makes it idempotent — run it 10 times, same result.
#             This is the layer analysts use for trend analysis.
# ================================================================================

# SILVER RULES:
#   ✅ Apply all business cleaning rules
#   ✅ Use MERGE not overwrite (idempotent)
#   ✅ Enable Change Data Feed (tracks changes)
#   ✅ Add processing_date column

# WHY MERGE NOT OVERWRITE:
#   Overwrite destroys Delta history and is not idempotent.
#   MERGE checks if record exists:
#     If yes → update it (SCD Type 1)
#     If no  → insert it
#   Run twice → same result. No duplicates. History preserved.

# WHAT YOU ARE BUILDING:
#   Two Silver Delta tables (rides and drivers) with
#   cleaning rules applied and idempotent MERGE writes.

# ────────────────────────────────────────────────────────────────────────────────
# Create Notebook: RideWave_Silver_Transformation
# Run on: Databricks → Workspace → Create Notebook → Serverless
# ────────────────────────────────────────────────────────────────────────────────

# ────────────────────────────────────────────────────────────────────────────────
# CELL 0 — Recovery Cell
# ────────────────────────────────────────────────────────────────────────────────
...
# YOUR_NAME   = "yourname"    # ← CHANGE TO YOUR FIRST NAME
...
# ────────────────────────────────────────────────────────────────────────────────
# CELL 1 — Read Bronze and Apply Silver Rules
# WHY: Reads from Bronze (raw) and applies all cleaning rules.
#      Never reads from source CSV directly — always from Bronze.
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
# CELL 2 — Write to Silver using MERGE
# WHY: MERGE is idempotent — run it 10 times, same result, no duplicates.
#      This is the industry standard for Silver layer writes.
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
# CELL 3 — Prove Idempotency
# WHY: Re-running the MERGE proves it is idempotent.
#      Count must stay the same — this is a KEY demo point.
# ────────────────────────────────────────────────────────────────────────────────
...
# WHY: Same MERGE pattern for drivers — every Silver table is idempotent.
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
# CELL 5 — Enable Change Data Feed
# WHY: CDF tracks every INSERT, UPDATE, DELETE on Silver.
#      Downstream Gold tables can read ONLY changed rows —
#      much faster than reading all Silver rows every time.
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
# CELL 6 — SQL Task S3: Window Functions in Silver
# WHY: Business analysts need running totals and rankings.
#      Window functions answer these without self-joins.
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
# CELL 7 — Spark Task SP2: Broadcast Join
# WHY: Broadcast join eliminates network shuffle for small tables.
#      Understanding this is critical for performance tuning.
# ────────────────────────────────────────────────────────────────────────────────
...
# EXPECTED OUTPUT:
#   rides_silver   : ~157 rows
#   drivers_silver : ~100 rows